...
def generate_heatmap(dirpath: str, filename: str, grid_rows: int, grid_cols: int) -> None:
...
def plot_histogram(hist: np.ndarray, bin_edges: np.ndarray, title: str, filename: str, mean: float | None, tail: float | None) -> None:
...
def plot_histogram_and_cumulative(dirpath: str, filename: str) -> None:
    df = read_csv(os.path.join(dirpath, 'logs', filename),
    ...
    plot_histogram(hist, bin_edges, filename.split('.')[0], os.path.join(
        dirpath, 'histograms', filename.split('/')[-1] + '.png'), df.iloc[:, 0].mean(), tail_avg)

    cumsum = np.cumsum(hist)
    plot_histogram(cumsum, bin_edges, filename.split('.')[0], os.path.join(
        dirpath, 'cumulative', filename.split('/')[-1] + '.png'), None, None)
...
def plot_execution_times(dirpath: str) -> None:
    ...
    plot_histogram(hist, bin_edges, 'Execution Times', os.path.join(
        dirpath, 'histograms', 'execution_times.png'), float(np.mean(times)), None)
